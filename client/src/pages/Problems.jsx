...
const Problems = () => {
	...
	const filteredByStatus = status => {
		setFilteredData(problems.filter(problem => problem.status === status))
	}

	const stats = Array.from(new Set(problems?.map(problem => problem.status)))
			...
			{!user ? (
				<p className='text-center text-3xl font-bold'>
					...
					<span className='text-xl font-semibold'>
						Станьте частью нашего портала, пройдя{' '}
			...
			{problems?.length === 0 ? (
				...
				<>
					<select
						...
						className='py-2 px-3 w-full text-center rounded-lg text-lg text-black font-semibold outline-none placeholder: placeholder:font-extralight md:w-1/3 md:mx-5'
					>
